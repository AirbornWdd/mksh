...
enum node_type 
{
	...
	NAC_NODE,			/* for nac */
	USER_NODE,			/* for user */
    VERSION_NODE,       /* for version. */
    PROJECT_NODE,       /* for project. */
    PRO_CONF_NODE,      /* project config. */
	VTY_NODE			/* Vty node. */
};
...
#define CMD_STR_BUFF   1024
#define CMD_DATA_BUFF  4096
...
int cmd_execute_system_command2(char *command);
int cmd_exec_system_cmd_pipe(char *command_producer,int argc_p, char **argv_p, 
...
void cmd_sort_node (void);
void cmd_parse_init(void);
int cmd_bridge_init(void);
int cmd_common_init(void);
int cmd_ip_init(void);
int cmd_tool_init(void);
int cmd_hook_init(void);
int cmd_trace_init(void);
void signal_init(void);
void trace_assert(void);
int cmd_slabtop_init(void);
int cmd_nicstat_init(void);
int cmd_pmap_init(void);
void cmd_dropstat_init(void);
...
extern char *ipproto_str[];
extern inline char *l3proto_str(unsigned short proto);
