...
static int vtysh_completion_entry_function (const char *text, int state)
{
    return 0;
}
...
static void vtysh_readline_init ()
{
    ...
    rl_completion_entry_function = (rl_compentry_func_t *)vtysh_completion_entry_function;
...
static char * vtysh_prompt ()
{
	...
	if (!username) {
        ...
        if (username) {
    		host.username = strdup (username);
        } else {
            username = "null";
        }
	}
    
	snprintf (buf, sizeof buf, cmd_prompt (vty->node), username, hostname);
